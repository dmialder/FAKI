...
class Film(object):
    ...
    def get_films(self):
        sql = text("SELECT film.id AS id, film.film_name AS film_name, film.film_year AS film_year;")
    ...
    def get_film_genres(self, film_id):
        sql = text(f"SELECT GROUP_CONCAT(genre, ', ')
    ...
    def get_film_countries(self, film_id):
        sql = text(f"SELECT GROUP_CONCAT(country, ', ')
    ...
    def get_people(self):
        sql = text(f"SELECT person.id, person.name, person.year_of_birth, year_of_death.year_of_death
    ...
    def get_person_films(self, person_id):
        sql = text(f"SELECT film.id, film.film_name, profession.professtion_name
            FROM film_person_profession
            JOIN film ON film_person_profession.film_id = film.id
            JOIN profession ON mpp.profession_id = profession.id
